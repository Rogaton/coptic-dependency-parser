    ...
    try:
        ...
        for sent_idx, stanza_sentence in enumerate(doc_tok.sentences, 1):
            ...
            if prolog and prolog.prolog_initialized:
                ...
                # Debug: Count punct relations in this sentence
                punct_count = sum(1 for rel in deprels if rel == 'punct')
                if punct_count > 0:
                    text_output_lines.append(f"\n[DEBUG] Found {punct_count} punct relations in sentence")
                    text_output_lines.append(f"[DEBUG] Validation returned {len(validation.get('warnings', []))} warnings")
                ...
                if validation.get("warnings"):
                    ...
                    text_output_lines.append(f"\n[DEBUG] No warnings generated by Prolog")
